/*2.12 ELIMINAR UN NODO DESPUES DE UN NODO CON INFORMACION X
1- realizar la verificacion si esta vacia , considerando si p->sig=NULL, si es asi entonces la lista esta vacia
2- considerando si es solo1  nodo o el nodo a eliminar se encuentra antes de NULL
...
nodo *eliminar_despues_x(nodo *,int);
...
int main()
{
	...
	do
	{
		...
		cout<<"\t11- Eliminar el nodo posterior con informacion X"<<endl;
		cout<<"\t13- S A L I R  \n\t";
		cin>>op;
		switch(op)
		{
			...
			case 11:
				{
					system("cls");
					cout<<"Ingrese el valor a buscar para eliminar su nodo posterior: ";
					cin>>ref;
...
nodo *eliminar_despues_x(nodo *p,int x)
{
	nodo *q,*t;
	int cen;
	if(p->sig!=NULL)
	{
		...
		else
		{
			if(q->sig!=NULL)
			{
				...
				cout<<"No existe un nodo posterior a "<<x<<endl;
		}
	}
	else
	{
