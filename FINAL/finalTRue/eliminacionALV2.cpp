...
class Arbol {
	...
	public:
	    ...
	    void mostrarMediana(Nodo* apnodo,Nodo* raizOriginal);
...
void Arbol::mostrarMediana(Nodo* apnodo,Nodo* raizOriginal){
	if (apnodo == NULL)
        return;
    ...
    if (apnodo != raizOriginal)
        cout << apnodo->info << " ";

    mostrarMediana(apnodo->izq, raizOriginal);
    mostrarMediana(apnodo->der, raizOriginal);
}
...
void menu(){
	...
	do{
		...
		cout<<"7. (PRUEBA) Mostrar Mediana"<<endl;
		cout<<"8. Contar nodos del arbol"<<endl;
		cout<<"0. Salir"<<endl;
		...
		switch(opcion){
			...
			case 7:{
				if(raiz==NULL){
					...
					arbol.mostrarMediana(raiz,raiz);
				}
				break;
			}
			case 8:{
				if(raiz==NULL){
					...
					cout<<"La Cantidad de nodos son: "<< arbol.contarNodos(raiz)<<endl;
				}
				break;
			}
			case 0:
