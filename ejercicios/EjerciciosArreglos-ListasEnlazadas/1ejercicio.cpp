...
void insertaAntesReferencial(arreglo *,int&, int);
...
int main(){
	...
	inserta(x,n,3);
	inserta(x,n,5);inserta(x,n,1);
	
	insertaAntesReferencial(x,n,7);
	mostrar(x,n);
	return 0;
}
...
void insertaAntesReferencial(arreglo *x,int &n, int ref){
	if(n<0){
		...
		if(n>=MAX){
			...
			if(pos<0){
				cout<<"No se encontro el "<< ref <<endl;
				return;
			}else{
				int dato2;
				cout<<"Ingrese el dato "<<endl;
